// routes/playerRoutes.js - REST API Routes for player management
...
router.get('/:profileId/history', [
  ...
  try {
    ...
    const games = await db('scores as sc')
      .join('sessions as s', 'sc.session_id', 's.id')
      .where('sc.player_profile_id', profileId)
      .select(
        'sc.score',
        'sc.submitted_at as played_at',
        's.room_code',
        's.id as session_id'
      )
      .orderBy('sc.submitted_at', 'desc')
      .limit(limit)
      .offset(offset);

    const total = await db('scores')
      .where('player_profile_id', profileId)
      .count('* as count')
      .first();
