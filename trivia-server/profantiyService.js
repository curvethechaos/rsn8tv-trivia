...
class ProfanityService {
  ...
  async check(texts) {
    try {
      ...
      for (const text of textsArray) {
        ...
        if (response.data.profanity && response.data.profanity.length > 0) {
  ...
  async clean(text) {
    try {
      ...
      return response.data.clean || text;
    } catch (error) {
  ...
  async checkFields(fields) {
    ...
    try {
      for (const [fieldName, value] of Object.entries(fields)) {
        ...
        const hasProfanity = response.data.profanity && response.data.profanity.length > 0;
